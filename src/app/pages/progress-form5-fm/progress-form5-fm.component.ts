...
@Component({
  selector: 'app-progress-form5-fm',
  templateUrl: './progress-form5-fm.component.html',
  styleUrls: ['./progress-form5-fm.component.scss', '../pagesStyle.css']
})
export class ProgressForm5FmComponent implements OnInit {
  ...
  GetUsers(id: any) {
    this.api.GetUserAll().subscribe((data: any) => {
      if (data.length > 0) {
        ...
        console.log(`⚡ ~ :469 ~ ProgressForm5FmComponent ~ this.Users:`, this.Users);
  ...
  async ResultSubmit() {
    ...
    if (ans == true) {
      ...
      let UserEmailList = [];
      let resUsers = await this.api.GetUserBySectionName(this.form.requestFormSectionName).toPromise()
      const filterData = resUsers.filter((item) => {
      ...
      console.log(`⚡ ~ :1105 ~ ProgressForm5FmComponent ~ resUsers:`, resUsers);


      let resUser2 = await this.api.GetSectionByName(this.form.requestFormSectionName).toPromise()
      const temp1 = resUser2.filter(item => item.view == 1)
      const cc = temp1[0].cc
      cc.forEach(element => {
        UserEmailList.push(element.Email)
      });
      const uniqEmail = [...new Set(UserEmailList)];
      let ccNew = uniqEmail.filter(item => item != this.Users.Email)

      console.log(`⚡ ~ :1122 ~ ProgressForm5FmComponent ~ ccNew:`, ccNew);
          ...
          this.api.UpdateResult(data[0]._id, ResultData).subscribe((data: any) => {
            if (data) {
              ...
              this.api.UpdateForm(this.formId, d).subscribe((data: any) => {
                ...
                if (data) {
                  const Content = "<p>To " + this.Users.FirstName + " " + this.Users.LastName + "</p><br>" +
  ...
  debugValue() {
    ...
    //   for (const key2 in this.ResultForm.controls[key]) {
    //     if(key2=='valid'){
    //       console.log(key, key2, this.ResultForm.controls[key][key2])
    //     }
    //   }
    // }
  }
