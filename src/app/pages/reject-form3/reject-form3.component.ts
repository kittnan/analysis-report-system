...
@Component({
  ...
  templateUrl: './reject-form3.component.html',
  styleUrls: ['./reject-form3.component.css', '../pagesStyle.css']
})
export class RejectForm3Component implements OnInit {

  constructor(
    // private api: RejectForm2Service,
    // private api: ViewFormService,
    // private api: ProgressForm3Service,
  ...
  ReplyDate = new FormControl(null);
  ...
  TreatmentOfNg = new FormControl(null);
  TreatmentList: any;
  ...
  getForm() {
    ...
    this.api.FindFormById(d).subscribe((data: any) => {
      ...
      if (data) {
        ...
        let temp1 = str[0];
        let temp2 = str2[0];
        this.IssueDate.setValue(temp1);
        this.ReplyDate.setValue(temp2);
  ...
  getResult() {
    ...
    this.api.FindResultByFormIdMain(id).subscribe((data2: any) => {
      if (data2.length > 0) {
        ...
        let temp2 = st2[0];
        let temp3 = st3[0];
        this.AnalyzeDate.setValue(temp1);
        this.ResultDate.setValue(temp2);
        this.ReportDate.setValue(temp3);
        // console.log(this.CategoryCause);
        this.GetCause();
  ...
  GetList(MasterId: any) {
    this.api.GetList(MasterId).subscribe((data: any) => {
      if (data.length > 0) {
        if (data[0].idMaster == this.SourceId) {
          this.SourceList = data;
        }
        if (data[0].idMaster == this.AnalysisLevelId) {
          this.AnalysisLevelList = data;
        }
      }
    })
  }
  ...
  ResultSubmit() {
    ...
    const ans = confirm("Do you want to Submit ?")
    if (ans == true) {
      ...
      this.api.UpdateResult(this.ResultId, d).subscribe((data: any) => {
        if (data) {
          const d1 = {
      ...
      if (result.isConfirmed) {
        ...
        if (this.FileListname.length > 0) {
          ...
          this.FileListname.forEach((item, index) => {
            ...
            this.api.RemoveFile(body).then((data: any) => {
              if (data) {
                let d = {
                  path: this.PathListName[index]
                }
                this.api.DeleteFileName(d).subscribe((data: any) => {
                })
              }
            })
  ...
  uploadFile(event: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        let tempName = this.form.requestNumber + "@" + event.target.files[0].name

        if (this.tempFile.filter(item => item.name == tempName).length == 0) {
          this.tempFileTotal += event.target.files[0].size;


          if (this.tempFileTotal < 31457280) {

            this.api.UploadFileEng(event.target.files[0], tempName).then((res) => {
              const setData = {
                path: res,
                name: tempName,
                size: event.target.files[0].size
              }
              this.tempFile.push(setData);
              ...
              this.api.UpdateResult(this.result._id, data).subscribe((data: any) => {
                if (data) {
